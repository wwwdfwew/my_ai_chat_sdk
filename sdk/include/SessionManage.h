...
class SessionManage {
public:
//在将会话信息存储到数据库中前，先要将数据库给创建好，在构造Session对象时创建数据库
SessionManage(const std::string& db_name="ChatSDK.db");
...
//构建消息id
std::string BuildMessageId(size_t messageCount);
private:
    ...
    //会话总数，须保证安全性
     std::atomic<int64_t> _session_counter;

    //持久化存储会话
    DataManager _dataManager;//数据管理类，用于持久化存储会话数据
};
